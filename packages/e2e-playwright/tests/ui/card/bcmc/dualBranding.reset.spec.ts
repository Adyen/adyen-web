...
test.describe('Testing Bancontact, with dual branded cards, how UI resets', () => {
    test('#1 should sort brands with Bcmc first and show only bcmc logo after deleting digits', async ({ bcmc }) => {
    ...
    test('#2 should keep BCMC branding when pasting unrecognised Visa number over dual branded card', async ({ bcmc }) => {
        await bcmc.goto(URL_MAP.bcmc);
    ...
    test('#3 should reset to BCMC branding and hide CVC after pasting unrecognised number over visa-selected dual branded card', async ({ bcmc }) => {
        await bcmc.goto(URL_MAP.bcmc);
