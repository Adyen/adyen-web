...
test.describe('Test how Custom Card Component with separate date field handles hidden expiryDate policy', () => {
    ...
    test.only('#2 validating fields first and then entering PAN should see errors cleared from state', async ({ customCardPageSeparate }) => {
        const { card, page } = customCardPageSeparate;

        await binLookupMock(page, hiddenDateAndCvcMock);

        await card.isSeparateComponentVisible(); // TODO - something not right: this line stops the pay button from being pressed
