...
test.describe('Card with 3DS2', () => {
    test.describe('Different 3DS2 flows', () => {
        ...
        test('should handle full flow (fingerprint & challenge) but call its own, component level, onAdditionalDetails callback', async ({
            page,
            card
        }) => {
            const makeDetailsCallResponsePromise = page.waitForResponse(response => response.url().includes('/details')); // Check for /details call
