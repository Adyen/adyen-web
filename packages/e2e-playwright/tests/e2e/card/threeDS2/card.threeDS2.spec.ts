...
test.describe('Card with 3DS2', () => {
    test.describe('Different 3DS2 flows', () => {
        ...
        test("should handle full flow (fingerprint & challenge) but call it's own, component level, onAdditionalDetails callback", async ({
            page,
            card
        }) => {
            const makeDetailsCallResponsePromise = page.waitForResponse(response => response.url().includes('/details')); // Check for sessions' /paymentDetails call

            await card.goto(URL_MAP.cardWithOwnOnAdditionalDetailsCallback);
            ...
            // Look for interim result generated by component level onAdditionalDetails callback"
            await expect(card.paymentResult).toHaveText('success-own-onAdditionalDetails-called');

            const detailsCallResponse = await makeDetailsCallResponsePromise;

            // Look for final authorised result
            await expect(card.paymentResult).toContainText(PAYMENT_RESULT.authorised);
            expect(detailsCallResponse.status()).toBe(200);
        });
