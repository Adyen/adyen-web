...
test(
    '#3 Enter card number (co-branded bcmc/visa) & fill expiryDate' +
        ' then select Visa and expect cvc input to be visible & comp to not be valid' +
        ' then select BCMC and expect cvc input to be hidden & comp to be valid',
    async ({ page, bcmc }) => {
        await bcmc.goto(URL_MAP.bcmc);
        ...
        await expect(bcmc.cvcInput).toHaveAttribute('aria-required', 'true');
...
test(
    '#4 Enter card number (co-branded bcmc/visa) & fill expiryDate' +
        ' then select Visa and expect comp to not be valid' +
