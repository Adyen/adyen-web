...
test.describe('#1 Error panel is present at start, when there are no errors, but is empty', async () => {
    // Wait for field to appear in DOM
    // error panel exists but is empty
});

test.describe('#2 Click pay with empty fields and error panel is populated', async () => {
    // Wait for field to appear in DOM
...
test.describe('#3 Fill out PAN & see that first error in error panel is date related', async () => {
    // Wait for field to appear in DOM
