...
- **Owns**: `Language.ts`, `LanguageService.ts`, `constants.ts`, `utils.ts`, `types.ts`, `index.ts`.
...
The script parses the array with a regex anchored on both `export const CDN_SUPPORTED_LOCALES = [`
and `] as const;`. Reformatting the opening line or dropping `as const` makes it throw before it
validates anything — the failure names the constant, not what you changed. Worse, it also parses
one `'locale',` per line, so collapsing the array onto a single line yields a silently empty list
instead of an error. Keep the formatting as-is.
