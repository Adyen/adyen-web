...
describe('Twint', () => {
    describe('isValid', () => {
        test('Is always valid', () => {
            const twintElement = new Twint(global.core);
    ...
    describe('displayName', () => {
        test('Says saved in title if stored payment method', () => {
            const twintElement = new Twint(global.core, { i18n: global.i18n, isStoredPaymentMethod: true });
            expect(twintElement.displayName).toBe('Twint saved');
        });

        test('Just say TWINT in title if not stored payment method', () => {
            const twintElement = new Twint(global.core, { i18n: global.i18n, name: 'TWINT' });
