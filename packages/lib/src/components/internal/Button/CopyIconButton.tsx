...
const CopyIconButton = (props: CopyIconButtonProps) => {
    ...
    const [tooltipText, setTooltipText] = useState<string>(i18n.get('button.copy'));
    const tooltipId = useRef(getUniqueId('adyen-checkout-tooltip'));
    const { anchorRef, visible, showTooltip, hideTooltip } = useTooltip<HTMLButtonElement>(TOOLTIP_VISIBILITY_DELAY);

    const onClick = useCallback(() => {
        void copyToClipboard(props.text);
        setTooltipText(i18n.get('button.copied'));
        showTooltip();
    ...
    useEffect(() => {
        if (!visible) {
            ...
            const timeout = setTimeout(() => setTooltipText(i18n.get('button.copy')), TOOLTIP_VISIBILITY_DELAY);
    ...
    return (
        <Fragment>
            ...
            <Tooltip text={tooltipText} anchorRef={anchorRef} id={tooltipId.current} visible={visible} />
        </Fragment>
    );
};
