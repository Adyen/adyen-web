...
export default function BSBInput({ setComponentRef, defaultData, placeholders, onChange, setStatus, id }: Readonly<BSBInputProps>) {
    ...
    const instructionId = useMemo(() => getUniqueId('bsb-instruction'), []);
    ...
    return (
        <Fieldset id={id} classNameModifiers={['payto__bsb_input']} label={'BSB'} description={'payto.bsb.description'} descriptionId={instructionId}>
            <Field
