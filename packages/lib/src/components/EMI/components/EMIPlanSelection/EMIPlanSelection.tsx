...
import { getSelectableIssuers } from '../../utils';
...
export function EMIPlanSelection({ issuers, selection, onSelectionChange, labelledBy, describedBy }: Readonly<EMIPlanSelectionProps>): h.JSX.Element {
    ...
    const issuerItems: SelectItem[] = getSelectableIssuers(issuers).map(issuer => {
    ...
    return (
        <div className={styles.planSelection} role={'group'} aria-labelledby={labelledBy} aria-describedby={describedBy}>
